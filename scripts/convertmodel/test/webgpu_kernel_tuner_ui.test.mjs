...
// kernels" panel, exactly the way a real visitor would click through it:
//
//   1. Load a real fixture (webgpu_tinygrad_conv3d.onnx, already carrying an
//      attached kernel spec on its one flagged Conv node) via the same
//      `window.webgpuKernelsShowBefore` hook Hugging Face loads use.
//   2. Click "Tune this kernel…" and wait for the real Pyodide+tinygrad
//      candidate generation and real WebGPU dispatch loop to finish --
//      needs real network access (jsdelivr for Pyodide, PyPI for tinygrad's
//      wheel), same requirement as the pyodide_webgpu_*_codegen.test.mjs
//      files, plus a real WebGPU device (Playwright/Chromium).
//   3. Click "Export tuned model" and capture the real browser download.
//   4. Verify the downloaded bytes actually carry the winning candidate's
...
//      dispatching *that* spec against a real device reproduces
//      webgpu_tinygrad_codegen_fixture.json's own expectedOutput -- so this
//      isn't just "a button produced a file", the exported model's attached
//      kernel is proven to still compute the right answer.
//
...
async function main() {
  console.log(`WebGPU kernel tuner UI check (real Pyodide + tinygrad + WebGPU) -- node ${CONV_FIXTURE.nodeName}\n`);
  ...
  try {
    ...
    console.log(`Loading ${CONV_FIXTURE.file} into the panel via window.webgpuKernelsShowBefore...`);
    ...
    await page.evaluate(
      ...
      { bytes: Array.from(modelBytes), name: CONV_FIXTURE.file },
    );

    const tuneButtonSelector = `#webgpu-kernels-content button[data-action="tune"][data-node="${CONV_FIXTURE.nodeName}"]`;
    await check("the panel shows a 'Tune this kernel' button for the flagged node", async () => {
      await page.waitForSelector(tuneButtonSelector, { timeout: 10_000 });
    });

    console.log("Clicking 'Tune this kernel…' -- this loads Pyodide + tinygrad's wheel over the network, can take a while...");
    ...
    // tests' own tinygrad-wheel-fetch-plus-dispatch budget.
    ...
    await check("tuning produced more than one candidate", () => {
    ...
    await check("every candidate's sampled output is finite (no NaN/broken bindings)", () => {
      for (const r of tuneResult.result.results) {
        assert.ok(r.sampleFinite, `candidate ${r.appliedOpts} produced a non-finite sample`);
      }
    });

    await check("results are sorted fastest first and the winner is the fastest", () => {
    ...
    // Read the downloaded bytes into memory before closing the page/context
    // -- Playwright's own temp download artifact isn't guaranteed to survive
    // the page it was downloaded on being closed.
    const exportedBytes = new Uint8Array(readFileSync(downloadPath));

    await page.close();

    await check("the exported file carries the winning candidate's spec", async () => {
      const { readWebgpuKernelSpecs } = await import("../onnx_node_metadata.mjs");
      const specs = readWebgpuKernelSpecs(exportedBytes);
      const attached = specs.get(CONV_FIXTURE.nodeName);
      assert.ok(attached, `no onnxsim.webgpu_kernel metadata found on ${CONV_FIXTURE.nodeName} in the exported model`);
    ...
    await check("the exported model's attached (tuned) kernel still computes the right answer", () => {
