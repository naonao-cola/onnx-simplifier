...
ImageNet top-1).
    ...
    python imagenette_data.py imagenette2-320 OUT_DIR
...
def normalize(x_uint8_nhwc):
    x = (x_uint8_nhwc.astype(np.float32) / 255.0 - MEAN) / STD
...
def main(root, out_dir):
    ...
    np.save(out / "val_x.npy", np.stack([load(p) for p, _ in val]))
    ...
    np.save(out / "calib_x.npy", normalize(np.stack([load(train[i][0]) for i in pick])))
