...
installed. For each batch size it writes into OUT_DIR:
    ...
    b{B}.raw.int8.onnx   ModelOpt INT8 (explicit Q/DQ) quantization of raw
    b{B}.sim.int8.onnx   ModelOpt INT8 quantization of the simplified model
...
def quantize_int8(src, dst, calib, input_name):
...
def main(argv=None):
    ...
    for batch in args.batch:
        ...
        for tag in ("raw", "sim"):
            try:
                t0 = time.perf_counter()
                quantize_int8(out / f"b{batch}.{tag}.onnx", out / f"b{batch}.{tag}.int8.onnx",
                              calib, input_name)
                n = sum(x.op_type == "QuantizeLinear"
                        for x in onnx.load(out / f"b{batch}.{tag}.int8.onnx").graph.node)
                print(f"b{batch}.{tag}.int8: {n} QuantizeLinear ({time.perf_counter() - t0:.1f}s)")
            except Exception:
                print(f"b{batch}.{tag}.int8: ModelOpt FAILED")
