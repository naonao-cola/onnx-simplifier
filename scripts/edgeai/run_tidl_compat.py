#!/usr/bin/env python3
"""Run the TIDL (TI edgeai accelerator) coverage heuristic over the model
suite and report.
...
Like ``scripts/axera/run_pulsar2_compat.py``, this needs no vendor package or
device -- see ``tidl_backend.py`` -- so there is no ``--require-*`` flag or
``skipped`` status: it always runs.

Usage:
    run_tidl_compat.py --output tidl-compat.csv
    run_tidl_compat.py --models conv_bn_relu matmul_bias_tanh
"""
...
def main() -> int:
    ...
    ap.add_argument("--output", default="tidl-compat.csv")
    args = ap.parse_args()

    selected = args.models or models.names()
    print(
        f"TIDL (TI edgeai accelerator) coverage check | {len(selected)} models",
        flush=True,
    )
    ...
    for i, name in enumerate(selected, 1):
        ...
        if r.get("coverage_orig") or r.get("coverage_simp"):
            detail += f", tidl={r.get('coverage_orig')}->{r.get('coverage_simp')}"
