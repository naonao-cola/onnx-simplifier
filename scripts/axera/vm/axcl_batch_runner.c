...
static void cmd_run(char *args) {
  ...
  for (uint32_t i = 0; i < n_in; i++) {
    ...
    void *h = host(m->in_size[i]);
    if (!p || !h || read_file(p, h, m->in_size[i])) {
...
int main(void) {
  ...
  while (fgets(line, sizeof line, stdin)) {
    ...
    else if (!strncmp(line, "RUN ", 4)) cmd_run(line + 4);
