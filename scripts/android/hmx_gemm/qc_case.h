...
static int qc_load_case(const char* dir, qc_case_t* c) {
  ...
  c->ref = qc_read(dir, "ref.bin", (size_t)c->M * c->N);
  c->w = qc_read(dir, "w.bin", (size_t)c->N * c->K);
  c->bq = qc_read(dir, "bq.bin", (size_t)c->N * 4);
  c->sw = qc_read(dir, "sw.bin", (size_t)c->N * 4);
  c->wk = malloc((size_t)c->K * c->N);
  c->wp = malloc((size_t)c->K * c->N);
  qc_transpose_w(c->w, c->N, c->K, c->wk);
  hmx_pack_w_u8cm(c->wk, c->K, c->N, c->wp);
  c->blk = aligned_alloc(256, sizeof(qc_blk_t) * (c->N / 32));
  qc_pack_params(c->wk, c->K, c->N, c->bq, c->zx, c->sx, c->sw, c->sy, c->zy, c->relu, c->blk, &c->hdr);
  return 0;
}

/* y: [M, N] row-major. Returns mismatches; hist[0..2] = counts of diff -1, +1, other */
