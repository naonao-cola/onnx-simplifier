...
#   | ./run.sh layers <flags> <M> <C> <L> [iters]
#   | ./run.sh health
...
case "$1" in
  setup)
    "${A[@]}" shell "mkdir -p $D"
    "${A[@]}" push "$B/hmx_gemm_client" "$B/hmx_gemm_u8_client" "$B/hmx_gemm_layers_client" "$B/hmx_gemm_rpc.so" "$MSDA_BUILD/msda_client" "$MSDA_BUILD/msda_rpc.so" $D/ >/dev/null
    "${A[@]}" push "$MSDA_CASE" "$D/case" >/dev/null
    "${A[@]}" shell "chmod 755 $D/hmx_gemm_client $D/hmx_gemm_u8_client $D/hmx_gemm_layers_client $D/msda_client" ;;
  gemm)
    shift
