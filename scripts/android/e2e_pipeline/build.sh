#!/bin/bash
# Build the one-process driver + the two DSP skels (sources from ../tinygrad_hexagon_bridge, used
...
mkdir -p "$B/rpn_fused" "$B/proposal_decode" "$B/topk" "$B/nms" "$B/roi"
...
# the driver (ARM64): ORT C++ API + the two qaic stubs + rpn_glue.c (rpn_model_io.h, unchanged)
CC="$NDK/aarch64-linux-android29-clang"
CXX="$NDK/aarch64-linux-android29-clang++"
RPC=(-I "$HEXAGON_SDK_ROOT/ipc/fastrpc/rpcmem/inc" -I "$B/rpn_fused" -I "$B/roi")
...
"$CXX" -O2 -std=c++17 -static-libstdc++ -I "$QS/headers" "${INC[@]}" "${RPC[@]}" -o "$B/e2e_run" "$HERE/e2e_run.cpp" \
  "$B/rpn_glue.o" "$B/rpn_stub.o" "$B/roi_stub.o" -L "$QS/libs" -lonnxruntime \
...
push "$QS"/libs/* "$B/e2e_run" "$B/rpn_fused/rpn_rpc.so" "$B/roi/roialign_rpc.so" \
