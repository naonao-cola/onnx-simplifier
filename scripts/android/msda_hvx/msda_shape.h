...
static inline int msda_shape_unpack(const int32_t* s, int n, msda_args_t* a) {
  ...
  a->mode = s[8]; a->NVR = s[9]; a->RL = s[10]; a->R = s[11]; a->RD = s[12];
  for (int l = 0; l < a->L; l++) {
...
static inline long msda_n_value(const msda_args_t* a) { return (long)a->NV * a->S * a->M * a->D; }
