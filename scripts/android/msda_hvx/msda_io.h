 ...
 *   meta.txt     "NV L S M D P Q NO mode NVR RL R RD has_vis vdtype", one "H W start" line per level,
 *                and for vdtype 1 (uint8 values) one "scale zero_point" line per value map
 *   value.f32 (vdtype 0) or value.u8 (vdtype 1), loc.f32 attw.f32 [ref.f32 unless mode 0]
 *   [vis.u8 if has_vis] ref_out.f32
...
static int msda_load(const char* dir, msda_case_t* c, void* (*alloc)(long)) {
  ...
  msda_args_t* a = &c->a;
  memset(a, 0, sizeof *a);
  int ok = fscanf(f, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d", &a->NV, &a->L, &a->S, &a->M, &a->D, &a->P, &a->Q,
                  &a->NO, &a->mode, &a->NVR, &a->RL, &a->R, &a->RD, &c->has_vis, &a->vdtype) == 15;
  for (int l = 0; ok && l < a->L && l < MSDA_MAX_L; l++) ok = fscanf(f, "%d %d %d", &a->H[l], &a->W[l], &a->start[l]) == 3;
  static float vscale[MSDA_MAX_NV];
  static int32_t vzp[MSDA_MAX_NV];
  for (int v = 0; ok && a->vdtype == MSDA_U8 && v < a->NV && v < MSDA_MAX_NV; v++) {
    int z = 0;
    ok = fscanf(f, "%f %d", &vscale[v], &z) == 2;
    vzp[v] = z;
  }
  fclose(f);
  if (!ok || msda_check(a)) { fprintf(stderr, "%s: bad meta\n", p); return -1; }
  const int u8 = a->vdtype == MSDA_U8;
  a->vscale = u8 ? vscale : 0;
  a->vzp = u8 ? vzp : 0;
  a->value = u8 ? 0 : alloc(msda_n_value(a) * 4);
  a->value_u8 = u8 ? alloc(msda_n_value(a)) : 0;
  ...
  if ((u8 ? !a->value_u8 : !a->value) || !a->loc || (a->mode != MSDA_LOC && !a->ref) || !a->attw || (c->has_vis && !a->vis) || !a->out ||
      !c->ref_out)
    return -1;
  return (u8 ? msda_read(dir, "value.u8", (void*)a->value_u8, msda_n_value(a))
             : msda_read(dir, "value.f32", (void*)a->value, msda_n_value(a) * 4)) ||
...
/* Tolerance for a body vs torch: fp32 math differs by rounding only; the HVX body's uint8 path also
 * rounds each tap weight to Q15 (<= 2^-16 absolute), up to ~2e-3 of max |out| on 100+ taps. */
static inline double msda_tol(const msda_args_t* a, int hvx) { return hvx && a->vdtype == MSDA_U8 ? 5e-3 : 5e-5; }
