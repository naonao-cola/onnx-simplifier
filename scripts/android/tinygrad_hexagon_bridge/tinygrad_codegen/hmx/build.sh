...
#   HEXAGON_SDK_ROOT=... HEXAGON_TOOLCHAIN=... TINYGRAD=<tinygrad hvx-hmx checkout> ./build.sh M K N [--i8]
set -euo pipefail
...
HMX=1 DEV=DSP MOCKDSP=1 TC=1 HVX_ARCH=v69 CC="${CC:-clang-19}" PYTHONPATH="$TINYGRAD" "$PY" "$SRC/gen_kernel.py" "$1" "$2" "$3" "$OUT" ${4:-}
cd "$OUT"
