...
the app's YOLO engine does for post=nms (cx,cy,w,h + class scores, per-class greedy NMS iou 0.7, conf 0.25, max 300)
...
def decode(head, conf=0.25, iou=0.7, max_det=300):
  head = head.reshape(84, -1)
  boxes, scores = head[:4].T, head[4:].T
...
def main():
  ...
  for i, p in enumerate(args.inputs):
    ...
    ref = decode(sess.run(None, {sess.get_inputs()[0].name: x})[0])
    nref += len(ref)
    for n, pat in runs:
      det = decode(np.fromfile(pat.format(i=i, stem=Path(p).stem), np.float32))
