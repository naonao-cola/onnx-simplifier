#!/usr/bin/env python3
"""Export one MapTR-tiny piece to ONNX, check it against the torch model, then onnxsim it
(the same flow as ../bevformer_tiny/export.py).

usage: export.py <piece> --ckpt <pth> --work <dir>
  pieces: backbone1 backbone6 encoder decoder
...
def piece(name, ckpt):
    ...
    if name == "encoder":
        return enc, ["feats", "can_bus", "ref_cam", "bev_mask"], ["bev"], \
            lambda f: (f["feats"], f["can_bus"], f["ref_cam"], f["bev_mask"])
    if name == "decoder":
        return dec, ["bev"], ["cls", "pts"], lambda f: (f["bev"],)
