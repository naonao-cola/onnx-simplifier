#!/bin/bash
# Build the DFA skel (dfa_rpc.idl / dfa_impl.c over ../../../msda_hvx/msda_kernel.h) and the
# s4d_run chain runner into $OUT.
...
OUT="${OUT:-$SRC/build}"
mkdir -p "$OUT" && cd "$OUT"
cp "$SRC"/dfa_rpc.idl "$SRC"/dfa_impl.c "$SRC"/dfa_core.h "$SRC"/s4d_run.cpp "$MSDA"/msda_kernel.h .
...
"$NDK_CXX" -O2 -std=c++17 -static-libstdc++ "${INC[@]}" -I "$HEXAGON_SDK_ROOT/ipc/fastrpc/rpcmem/inc" -I "$Q/headers" \
  -o s4d_run s4d_run.cpp dfa_stub.o -L "$Q/libs" -lonnxruntime \
  -L "$HEXAGON_SDK_ROOT/ipc/fastrpc/remote/ship/android_aarch64" -lcdsprpc
echo "built $OUT/dfa_rpc.so $OUT/s4d_run"
