 ...
 *   hexagon-sim -mv69 --simulated_returnval dfa_sim.elf -- <case dir> [w16 | w2] */
#include <hexagon_sim_timer.h>

#include "dfa_io.h"

int main(int argc, char** argv) {
  if (argc < 2) { printf("usage: dfa_sim <case dir> [w16]\n"); return 2; }
  dfa_args_t a;
  memset(&a, 0, sizeof a);
  dfa_case_load(argv[1], &a);
  uint16_t* stage = NULL;  /* argv[2]: "w16" fp16 weights, "w2" the v2 layout */
  float* wbuf = argc > 2 && !strcmp(argv[2], "w16") ? dfa_use_w16(&a)
                : argc > 2 && !strcmp(argv[2], "w2") ? dfa_use_w2(&a, &stage) : NULL;
