...
def push_runtime(work: Path, build: Path):
    ...
    for p in PIECES:
        subprocess.run([*E.ADB, "push", "-q", str(work / "msda_split" / f"{p}.sim.onnx"), f"{R}/msda_split/{p}.onnx"],
                       check=True)
...
def run_encoder(work: Path, inputs: dict, reps: int):
    ...
    subprocess.run([*E.ADB, "shell", f"rm -rf {R}/msda_frame && mkdir -p {R}/msda_frame"], check=True)
    subprocess.run([*E.ADB, "push", "-q", *[str(p) for p in fd.iterdir()], f"{R}/msda_frame/"], check=True)
    out = subprocess.run([*E.ADB, "shell", f"cd {R} && LD_LIBRARY_PATH={R} QNN_PERF=burst "
                          f"ADSP_LIBRARY_PATH='{R};/vendor/dsp/cdsp;/vendor/lib/rfsa/adsp;/system/lib/rfsa/adsp;/dsp' "
                          f"./enc_run msda_split 2 {reps} msda_frame 2>&1"], capture_output=True, text=True).stdout
    if "PASS" not in out:
        raise RuntimeError(out[-1500:])
    subprocess.run([*E.ADB, "pull", "-q", f"{R}/msda_frame/bev.f32", str(fd / "bev.f32")], check=True)
...
def main():
    ...
    ap.add_argument("--build", default=str(HERE / "build"), help="build.sh OUT (enc_run, msda_rpc.so)")
    ...
    for i, tok in enumerate(ns.scene_samples(a.scene)[: a.frames]):
        ...
        enc_in = (torch.from_numpy(feats_h), torch.from_numpy(p_h), has_prev, shift, can_bus, ref_cam, bev_mask)
        tsa_ref, ref_cam_h, vis = S.host_inputs(enc_in)
        f32 = lambda t: np.ascontiguousarray(np.asarray(t, np.float32))  # noqa: E731
        bev_h, report = run_encoder(work, {"feats": f32(feats_h), "prev_bev": f32(p_h), "has_prev": f32(has_prev),
                                           "can_bus": f32(can_bus), "tsa_ref": f32(tsa_ref), "ref_cam": f32(ref_cam_h),
                                           "vis": np.ascontiguousarray(vis.numpy().astype(np.uint8))}, a.reps)
        t_e = float(re.search(r"encoder_ms median ([0-9.]+)", report).group(1))
        enc_cos = E.cos(enc(*enc_in), bev_h)  # the encoder alone: fp32 torch on the phone's own inputs
        enc_ms.append(t_e)
        if i == 0:
        ...
        for k, (c, b) in {"cpu": (cls, bbox), "htp": (torch.from_numpy(cls_h), torch.from_numpy(bbox_h))}.items():
            ...
            for j in range(3):
                tot[k][j] += r[k][j]
        print(f"frame {i}: ms backbone {t_b:.0f} encoder {t_e:.1f} decoder {t_d:.0f} | cos vs fp32: feats "
              f"{E.cos(feats, feats_h):.5f} bev {E.cos(bev, bev_h):.5f} (encoder alone {enc_cos:.6f}) cls {E.cos(cls, cls_h):.5f} "
              f"bbox {E.cos(bbox, bbox_h):.5f} | matched/pred/GT fp32 {r['cpu']} phone {r['htp']}", flush=True)
