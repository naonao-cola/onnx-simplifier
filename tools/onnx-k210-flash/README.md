...
| Camera demo (`firmware/camera_demo/`) | **M5StickV: run on real hardware**, live camera -> KPU (Sobel edge model, 13 fps) -> LCD, colours and edge map checked by eye. **Maix Cube: power/camera/buttons confirmed on real hardware, LCD picture wrong colour** (genuine octal SPI reaches the panel and fills it evenly, but with the wrong hue -- a pixel-format bug, not structural) -- see `firmware/camera_demo/README.md`'s Cube status section. Numeric comparison with onnxruntime not done on either board; browser flasher not tried |
