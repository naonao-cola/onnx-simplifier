...
**TVM-generated (no hand-written intrinsics):** `hexagon_qfloat.py` is a TIR pass that turns ordinary vectorized multiply-accumulate chains into qfloat intrinsic chains; with plain TE schedules it reaches 15.8 ms (fp32), 6.06 ms (fp16 in/out, fp32 accumulate) and 6.8 ms (chunked qf16) versus 29.2/30.9/13.4 ms on stock TVM (`bench_tvm_hexagon_conv_transpose_generated.py`).
