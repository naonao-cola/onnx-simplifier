...
std::string ReplaceAllOccurrences(std::string text, const std::string& token,
...
// Picks the fixed expert-block template matching (activation, has_fc1_bias,
...
// and round-tripped through onnx's own text-format parser/checker/shape
// inference when the generator ran -- rather than hand-typed here; see that
// script's module docstring for why generating it is a separate, checked-in
// step instead of a build-time one. `activation` is assumed to already be
// one of the four BuildMoEFunctionBody below accepts.
const char* SelectMoEExpertBlockTemplate(const std::string& activation,
                                         bool has_fc1_bias, bool has_fc2_bias) {
  if (activation == "relu") {
    if (has_fc1_bias) {
      return has_fc2_bias ? kMoEExpertBlockReluFc1BiasFc2Bias
                          : kMoEExpertBlockReluFc1BiasFc2NoBias;
    }
    return has_fc2_bias ? kMoEExpertBlockReluFc1NoBiasFc2Bias
                        : kMoEExpertBlockReluFc1NoBiasFc2NoBias;
  }
  if (activation == "identity") {
    if (has_fc1_bias) {
      return has_fc2_bias ? kMoEExpertBlockIdentityFc1BiasFc2Bias
                          : kMoEExpertBlockIdentityFc1BiasFc2NoBias;
    }
    return has_fc2_bias ? kMoEExpertBlockIdentityFc1NoBiasFc2Bias
                        : kMoEExpertBlockIdentityFc1NoBiasFc2NoBias;
  }
  if (activation == "silu") {
    if (has_fc1_bias) {
      return has_fc2_bias ? kMoEExpertBlockSiluFc1BiasFc2Bias
                          : kMoEExpertBlockSiluFc1BiasFc2NoBias;
    }
    return has_fc2_bias ? kMoEExpertBlockSiluFc1NoBiasFc2Bias
                        : kMoEExpertBlockSiluFc1NoBiasFc2NoBias;
  }
  // "gelu".
  if (has_fc1_bias) {
    return has_fc2_bias ? kMoEExpertBlockGeluFc1BiasFc2Bias
                        : kMoEExpertBlockGeluFc1BiasFc2NoBias;
  }
  return has_fc2_bias ? kMoEExpertBlockGeluFc1NoBiasFc2Bias
                      : kMoEExpertBlockGeluFc1NoBiasFc2NoBias;
}

// Appends one expert's FFN block (Gemm -> activation -> Gemm, weighted by
// that expert's dense gate column and accumulated into `Acc<expert>`) to
// `text`, by substituting the expert index into the fixed template
// SelectMoEExpertBlockTemplate() returns. `expert` is baked in as a literal
// index -- the loop that calls this once per expert, in BuildMoEFunctionBody
// below, is what actually unrolls the op; nothing here is a runtime/ONNX-
// level loop.
void AppendMoEExpertBlock(std::ostringstream& text, int64_t expert,
                          bool has_fc1_bias, bool has_fc2_bias,
                          const std::string& activation) {
  std::string block =
      SelectMoEExpertBlockTemplate(activation, has_fc1_bias, has_fc2_bias);
  block =
      ReplaceAllOccurrences(std::move(block), "{{E}}", std::to_string(expert));
  block = ReplaceAllOccurrences(std::move(block), "{{E1}}",
                                std::to_string(expert + 1));
  text << block;
}
...
bool BuildMoEFunctionBody(const FunctionBodyBuildContext& ctx,
  ...
  // The routing head (Softmax over the raw routing logits `router_probs`
  // despite its name -- confirmed against ONNX Runtime's own CPU MoE
  // kernel, see kMoERoutingHeadNormalize/NoNormalize's own generator
  // comments in generate_moe_function_templates.py -- top-k selection,
  // optional renormalization, and the dense mostly-zero gate) is one of
  // the two fixed templates in contrib_schemas_moe_templates.gen.h,
  // generated from real onnxscript function definitions rather than
  // hand-typed here; `normalize` picks which one (they differ structurally
  // -- extra ReduceSum/Div nodes -- not just by a literal value).
  std::ostringstream text;
  std::string routing_head =
      normalize ? kMoERoutingHeadNormalize : kMoERoutingHeadNoNormalize;
  routing_head = ReplaceAllOccurrences(
      std::move(routing_head), "{{HIDDEN_SIZE}}", std::to_string(hidden_size));
  routing_head = ReplaceAllOccurrences(std::move(routing_head), "{{K}}",
                                       std::to_string(k));
  text << routing_head;

  for (int64_t e = 0; e < num_experts; ++e) {
    AppendMoEExpertBlock(text, e, has_fc1_bias, has_fc2_bias, activation);
  }
