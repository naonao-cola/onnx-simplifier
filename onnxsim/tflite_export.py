...
def _op_mod(lowerer, node, ins, attrs):
    ...
    # natively, with no float casts that would lose int precision.
    tf = lowerer.tf
    a, b = ins[0].t, ins[1].t
...
def _op_expand(lowerer, node, ins, attrs):
    ...
    # and accepts a dynamic shape tensor, so no const shape is required.
    tf = lowerer.tf
    x, shape = ins
...
def _op_topk(lowerer, node, ins, attrs):
    ...
    # 64-bit-to-32-bit downcast convention (see _NP_DOWNCAST).
    tf = lowerer.tf
    x = ins[0]
    x_shape = x.t.shape.as_list()
    rank = len(x_shape)
...
def _op_layer_norm(lowerer, node, ins, attrs):
    ...
    # stash_type only affects those outputs' dtype.
    tf = lowerer.tf
    x = ins[0]
    x_shape = x.t.shape.as_list()
    rank = len(x_shape)
...
def _op_scatter_nd(lowerer, node, ins, attrs):
    ...
    # anything else has no TF/TFLite primitive and fails loudly.
    tf = lowerer.tf
    ...
    if backend == "onnx2tf":
        ...
        if (
            ...
            raise TypeError(
                ...
                "int8_quantize/representative_dataset/inference_io_dtype/flex_ops; "
